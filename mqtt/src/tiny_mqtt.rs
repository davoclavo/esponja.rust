...
use esp_wifi::{wifi::WifiError, wifi_interface::Socket};
...
pub struct TinyMqtt<'a, MODE: WifiDeviceMode> {
    client_id: &'a str,
    socket: Socket<'a, 'a, MODE>,
    queue: RefCell<Queue<(usize, [u8; 1024]), 16>>,
    recv_buffer: [u8; 1024],
    recv_index: usize,
    recv_queue: RefCell<Queue<PacketBuffer, 16>>,
    timeout_secs: u16,
...
impl<'a, MODE: WifiDeviceMode> TinyMqtt<'a, MODE> {
    pub fn new(
        client_id: &'a str,
        socket: Socket<'a, 'a, MODE>,
        current_millis_fn: fn() -> u64,
        ...
        let res = TinyMqtt {
            ...
            recv_index: 0,
            recv_queue: RefCell::new(Queue::new()),
            timeout_secs: 0,
            last_sent_millis: 0,
            current_millis_fn,
            receive_callback,
    ...
    pub fn connect(
        ...
        let connect = Packet::Connect(Connect {
            ...
            client_id: "esponja", //self.client_id(),
            clean_session: true,
            last_will: None,
            username,
            password,
    ...
    pub fn poll_internal(&mut self, drain_receive_queue: bool) -> Result<(), TinyMqttError> {
        ...
        self.send_internal()?;
        // TODO Enable this once we figure out the self.socket.read blocking issue
    ...
    fn receive_internal(&mut self) -> Result<(), TinyMqttError> {
        loop {
            ...
            println!("try reading");
            // TODO Fix this as it blocks until there are bytes, so never returns 0
...
impl<'a, MODE: WifiDeviceMode> Mqtt for TinyMqtt<'a, MODE> {
    fn send(&self, packet: mqttrust::Packet<'_>) -> Result<(), mqttrust::MqttError> {
        println!("sending MQTT packet: {:?}", packet);
        let mut buf = [0u8; 1024];
        let len = encode_slice(&packet, &mut buf).unwrap();
